...
impl ProfileProcessor {
    ...
    fn process_trimmed_conic(
        ...
        // Heuristic: if any provided trim is outside a full turn in radians, interpret provided
        // trim values as degrees (including single-trim cases).
        let likely_degrees = start_raw
            .is_some_and(|value| value.abs() > (2.0 * PI + 1e-6))
            || end_raw.is_some_and(|value| value.abs() > (2.0 * PI + 1e-6));

        let to_angle = |value: Option<f64>, default_value: f64| -> f64 {
            match value {
                Some(v) if likely_degrees => v.to_radians(),
                Some(v) => v,
                None => default_value,
            }
        };

        let start_angle = to_angle(start_raw, 0.0);
        let end_angle = to_angle(end_raw, 2.0 * PI);
...
"#;
    ...
    #[test]
    fn test_trimmed_circle_single_trim_degree_fallback() {
        let content = r#"
#1=IFCCARTESIANPOINT((0.0,0.0));
#2=IFCAXIS2PLACEMENT2D(#1,$);
#3=IFCCIRCLE(#2,10.0);
#4=IFCTRIMMEDCURVE(#3,(IFCPARAMETERVALUE(270.0)),$,.T.,.PARAMETER.);
"#;
