...
use crate::{
    profiles::{segments_for_arc, segments_for_radius, ProfileProcessor},
...
impl GeometryProcessor for RevolvedAreaSolidProcessor {
    fn process(
        ...
        let max_radius = profile_2d
            .outer
            .iter()
            .map(|point| point.x.abs())
            .fold(0.0f64, f64::max);
        let segments = segments_for_arc(max_radius, angle.abs(), self.deflection);
