...
function generateGeometryCategories(schema: ExpressSchema): string {
  // Find geometry-related entities based on inheritance
  const geometryTypes = findGeometryTypes(schema);
  const profileTypes = findProfileTypes(schema);
  const spatialTypes = findSpatialTypes(schema);
  const productTypes = findProductTypes(schema);
...
`;
  ...
  code += `        )
    ...
    pub fn is_building_element(&self) -> bool {
        // Check name pattern for building elements
        // This avoids needing the abstract IfcBuildingElement in the enum
        let name = self.name();
        matches!(
            *self,
...
// Helper functions to find type categories
function findGeometryTypes(schema: ExpressSchema): EntityDefinition[] {
  return schema.entities.filter((e) => {
    const chain = getInheritanceChain(e, schema);
    return (
      chain.includes('IfcGeometricRepresentationItem') ||
      e.name.includes('Solid') ||
      e.name.includes('Brep')
    );
  });
}

function findProfileTypes(schema: ExpressSchema): EntityDefinition[] {
  return schema.entities.filter((e) => e.name.includes('ProfileDef'));
}

function findSpatialTypes(schema: ExpressSchema): EntityDefinition[] {
  return schema.entities.filter((e) => {
    const chain = getInheritanceChain(e, schema);
    return chain.includes('IfcSpatialStructureElement') || chain.includes('IfcSpatialElement');
  });
}

function findProductTypes(schema: ExpressSchema): EntityDefinition[] {
  return schema.entities.filter((e) => {
    const chain = getInheritanceChain(e, schema);
    return chain.includes('IfcProduct');
  });
}
