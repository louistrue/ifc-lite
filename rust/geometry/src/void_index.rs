...
/// those void relationships to every child part that has geometry (i.e. has a
...
pub fn propagate_voids_to_parts(
    ...
    // Collect parent→children mappings where parent has voids
    ...
    while let Some((id, type_name, start, end)) = scanner.next_entity() {
        ...
        // Only propagate if the parent actually has voids
        if !void_index.contains_key(&parent_id) {
            continue;
        }

        // Get child entity IDs from the RelatedObjects list (attr 5)
        ...
        // Filter to children that are IfcBuildingElementPart with geometry
        ...
        for child_id in children {
            if let Ok(child) = decoder.decode_by_id(child_id) {
                if child.ifc_type == IfcType::IfcBuildingElementPart {
                    // Check that the child has a representation (attribute 6)
    ...
    // Apply propagations: copy parent's voids to each eligible child
